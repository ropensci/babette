...
## Creating files to use

To make the examples simpler, I create all files locally here:
...
```{r}
posterior <- get_bbt_run_test_output()
posterior$test_output_0_trees <- posterior$anthus_aco_trees
posterior$my_fasta_trees <- posterior$anthus_aco_trees
posterior$my_alignment_trees <- posterior$anthus_aco_trees
posterior$estimates$BDBirthRate <- posterior$estimates$birthRate
names(posterior)
```
...
We will re-create this MCMC setup, as doing so initializes it with new filenames for
temporary files. These temporary files should not exist before a run and should exist
