...
inference_model$mcmc$tracelog$filename <- get_beautier_tempfilename()
inference_model$mcmc$treelog$filename <- get_beautier_tempfilename()
```
...
if (is_beast2_installed()) {
  ...
  testthat::expect_true(file.exists(beast2_options$state_filename))
}
```
...
```{r}
knitr::kable(head(parse_beast_log(inference_model$mcmc$tracelog$filename)))
```
