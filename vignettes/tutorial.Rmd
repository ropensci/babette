...
This will trigger autocomplete to show all site models. 

The simplest site model is the Jukes-Cantor DNA substutition model.
...
This will trigger autocomplete to show all clock models. 
...
This will trigger autocomplete to show all tree priors. 
...
create a prior distribution of the crown age, after wich we can
...
`posterior_crown_age` is the `bbt_run` argument to specify a fixed crown age:
...
Prefer to use an MRCA prior with a sharp distribution around the desired crown
...
`beast2_jar_path` is the `bbt_run` argument to specify the path 
